 ...
 * V2.59.3_DS - Clean Debug Messages
 * V2.59.3_KT414_R0 - printnb, tX/rx statistic support
 * V2.59.3_KT414_R1 - kernel usb_hcd_map support upgrade
 */

//#define DEBUG
#define LNX_DM9620_VER_STR  "V2593(KT414_R2)"

#define DM_HCD_MAP_KT4x // XX_CONF_KT4x features: new usbnet interface .. etc.
#define DM_CONF_KT3x

#ifdef DM_CONF_KT3x
#undef DM_HCD_MAP_KT4x
#endif
...
/* hcd map support */
#ifdef DM_HCD_MAP_KT4x //dm_usbmap_read
#define LNX_CONF0_STR	"LNX HCD MAP: On"
#define dm_usbmap_read	usbnet_read_cmd
#define dm_usbmap_write	usbnet_write_cmd
#else
#define LNX_CONF0_STR	"LNX HCD MAP: Off"
#define dm_usbmap_read	usb_control_msg
#define dm_usbmap_write	usb_control_msg
#endif 
...
#define TGL_EXTRA_WRITE 6 // Relaese: want > 6, 18, [25], 250, 500
	...
	int  IP_toggle_count;
	u8  IP_TOGG;
	u8  allow_defer_happen;    //u8  IP_done=
	u8  mdat[512];
};     
...
#else
#define dm9620_print(dev, format, args...) 
...
#ifdef DM_HCD_MAP_KT4x  //dm_usbmap_read
	int err;
	err= usbnet_read_cmd(dev, USB_REQ_GET_DESCRIPTOR,
			       USB_DIR_IN,
			       dt << 8, 0, data, size);
	if(err != size && err >= 0)
		err = -EINVAL;
...
static int dm_read_reg(struct usbnet *dev, u8 reg, u8 *value)
{
	...
	ret = dm_read(dev, reg, 2, tmpwPtr);  // usbnet_read_cmd/usbnet_write_cmd v.s. usb_submit_urb v.s. usb_control_msg
...
{
#ifdef DM_HCD_MAP_KT4x  //dm_usbmap_write
	int err;
...
{
#ifdef DM_HCD_MAP_KT4x  //dm_usbmap_write_async
...
//Stone add
	...
	ret = dm_read(dev, DM_SHARED_DATA, 2, tmpwPtr1);  // usbnet_read_cmd/usbnet_write_cmd v.s. usb_submit_urb v.s. usb_control_msg
...
void ne_desc_init(int enab)
{
  ne.enab_endpoint = 0; // default 0, to check to latest kernel if to use.
  ne.enab_desc = enab;
}
    
void ne_init(int enab)
{
  ne.enab = enab;
}

void ne_basic_varibles(struct usbnet *dev)
{
  ...
  priv->IP_conf_inc= CONF_INC;
}

#if 0
//int ne_endpoints(struct usbnet *dev, struct usb_interface *intf)
//{if (!ne.enab_endpoint)
//    return usbnet_get_endpoints();
//}
//void ne_desc(struct usbnet *dev) //major
//{   if (!ne.enab_desc)
//    return;
//}
#endif
int ne_endpoints(struct usbnet *dev, struct usb_interface *intf)
{
 ...
 printnb("\n#\n");
 printnb("(usbnet)DM961: Interface: num_altsetting= %d\n", intf->num_altsetting);
 ...
 printnb("(usbnet)DM961: Interface: altsetting: %d in addr: (struct usb_host_interface *) 0x%x\n", tmp, (unsigned int) alt); //long 
 ...
 printnb("(usbnet)DM961: Interface,altsetting: bNumEndpoints= %d\n", alt->desc.bNumEndpoints);
 ...
 printnb("(usbnet)DM961: Interface,altsetting,endpoint: Endpoints-number-%d\n", e->desc.bEndpointAddress & 0x0f); // D[3:0]= Endpoint Number
 
 printnb("(usbnet)EP%d: %02x %02x %02x %02x %02x %02x %02x\n", 
   e->desc.bEndpointAddress & 0x0f,
 	...
 	e->desc.bInterval);
			...
			case USB_ENDPOINT_XFER_INT:
			  ...
			  printnb("(usbnet)DM961: usb_endpoint_dir_in( ) is %d\n", (e->desc.bEndpointAddress & USB_ENDPOINT_DIR_MASK) == USB_DIR_IN );
			  printnb("Because e->desc.bEndpointAddress= %02x, USB_ENDPOINT_DIR_MASK= %02x, USB_DIR_IN= %02x\n",
			    e->desc.bEndpointAddress, USB_ENDPOINT_DIR_MASK, USB_DIR_IN);
			...
			if (usb_endpoint_dir_in(&e->desc)) {
				...
				{
					status = e;
					printnb("(usbnet)DM961: (struct usb_host_endpoint *) 0x%x\n", (unsigned int) status); //long 
					printnb("(usbnet)DM961: THis is ----- (usb_host_endpoint).interrupt ------\n");
				}
			} else {
				if (!out)
					out = e;
	...
	if (status){
	  dev->status = status;
	  printnb("(usbnet)DM961: dev->status = status (= 0x%x)\n", (unsigned int) status); //long 
	}
	
	if (!alt || !in || !out) {
 printnb("(usbnet)DM961: Return (!alt || !in || !out)\n");
		return -EINVAL;
	}
 ...
 printnb("(usbnet)DM961: interface (desc.bInterfaceNum: %d, desc.bAltSetting: %d)\n", 
 ...
 printnb("(usbnet)DM961: usb_rcvbulkpipe (In bEndpointAddress: %d)\n", in->desc.bEndpointAddress & USB_ENDPOINT_NUMBER_MASK);
	dev->in = usb_rcvbulkpipe (dev->udev,
			in->desc.bEndpointAddress & USB_ENDPOINT_NUMBER_MASK);
 printnb("(usbnet)DM961: usb_sndbulkpipe (Out bEndpointAddress: %d)\n", out->desc.bEndpointAddress & USB_ENDPOINT_NUMBER_MASK);
...
void ne_desc(struct usbnet *dev)
{	  
	  int i, n, m;
	  u16 integrate_size;
	  struct dm96xx_priv*priv = dev->driver_priv;
	 ...
	 printnb("\n");
	 printnb("++++++[Device desc]+++++  Get len= %d\n", n);
	 printnb(" %02x %02x %02x %02x %02x %02x %02x %02x  %02x %02x %02x %02x %02x %02x %02x %02x\n", 
	 ...
	 printnb("++++++[Config0 desc]+++++  Get len= %d\n", n);
	 for (i=0; i<9; i++)
	 {
		 if (i && (!(i%16)) ) printnb("\n");
		 else if (i && (!(i%8)) ) printnb(" ");
		 printnb(" %02x", priv->mdat[i]);
	 }
	 printnb("\n");
#endif
	 ...
	 printnb("++++++[Config0 Desc: %02x %02x]+++++  Total Descriptor Length len= %d\n",
...
//    return;
//}
//void ne_render(struct usbnet *dev) // major
//{   if (!ne.enab)
//    return;
//}
//void ne_opnum_reset(struct usbnet *dev)
//{   if (!ne.enab)
//    return;
//}
//void ne_opnum_count(struct usbnet *dev)
//{   if (!ne.enab)
//    return;
//}
//void ne_scan(struct usbnet *dev, struct sk_buff *skb) //major
//{   if (!ne.enab)
//    return;
//}
#else
...
static void Write_cnf_inc(struct usbnet *dev, u8 conf_stat)
{
	struct dm96xx_priv* priv = (struct dm96xx_priv *)dev->driver_priv;
		    ...
		    if ( (priv->mdat[0]==conf_stat) && (priv->mdat[1]==priv->IP_conf_inc))
			  printnb("++++++[INC already %d equal to EEPROM's %d]+++++\n", priv->IP_conf_inc, priv->mdat[1]);
		    else {
		    	
		      //if (priv->mdat[1]==0xff){
			  //  printnb("++++++[INC is change from 0xff to %d]+++++\n", priv->IP_conf_inc);
		      //  priv->mdat[1]= CONF_INC;
		      //} else
			    printnb("++++++[INC is written (INC cnf-stat 0x%02x)]+++++\n", conf_stat);
			    printnb("++++++[INC is change from %d to %d (0x%02x to 0x%02x)]+++++\n", 
...
void ne_render(struct usbnet *dev)
{
  struct dm96xx_priv*priv = dev->driver_priv;
  ...
  //dm_eeprom_render(dev, 3, 0x4010, 0xc030);[Test: 5551h]
    ...
    {
	  printnb("++++++[rdWORD 3]+++++  %02x %02x\n", priv->mdat[0], priv->mdat[1]);
	  priv->mdat[1] = 0x55;
	  priv->mdat[0] = 0x51;
	  dm_write_eeprom_word(dev, 6 / 2, priv->mdat);
	  udelayEx(10);
	  printnb("++++++[wrWORD 3]+++++  %02x %02x\n", priv->mdat[0], priv->mdat[1]);
	  udelayEx(100);
      dm_read_eeprom_word(dev, 6 / 2, priv->mdat);
	  printnb("++++++[rdWORD 3].ck1-+++++  %02x %02x\n", priv->mdat[0], priv->mdat[1]);
	  udelayEx(100);
      dm_read_eeprom_word(dev, 6 / 2, priv->mdat);
	  printnb("++++++[rdWORD 3].ck2-+++++  %02x %02x\n", priv->mdat[0], priv->mdat[1]);
	  udelayEx(100);
      dm_read_eeprom_word(dev, 6 / 2, priv->mdat);
	  printnb("++++++[rdWORD 3]+++++  %02x %02x\n", priv->mdat[0], priv->mdat[1]);
    }
  ...
  //dm_eeprom_render(dev, 3, 0x0010, 0x0030);[Keep: 1551h or 5551h]
    ...
    if (priv->mdat[1]&0x04)
    {
	  printnb("++++++[rdWORD 7]+++++  %02x %02x\n", priv->mdat[0], priv->mdat[1]);
	  priv->mdat[1] &= 0xfb;
	  dm_write_eeprom_word(dev, 14 / 2, priv->mdat);  
	  udelayEx(10);
	  printnb("++++++[wrWORD 7]+++++  %02x %02x\n", priv->mdat[0], priv->mdat[1]);
	  udelayEx(100);
...
void ne_scan(struct usbnet *dev, struct sk_buff *skb)
{
  struct dm96xx_priv*priv = dev->driver_priv;
...
static int dm9620_bind(struct usbnet *dev, struct usb_interface *intf)
{
	...
	ne_desc_init(1);  // 1/0 if print endpoints desc. (also 1 for 'ne_endpoints' itself, 0 for using 'usbnet_get_endpoints')
	ne_init(0);  // 1/0 for runtime new_cfg_inc (New_cnf_inc)

	//ret = usbnet_get_endpoints(dev, intf);
	ret = ne_endpoints(dev, intf);
	if (ret)
		goto out;
    ...
    /* bind version info.s */
	printnb("\n");
	printnb("[dm962] Linux Driver = %s\n", LNX_DM9620_VER_STR);
	printnb("[dm962] %s", LNX_CONF0_STR);
	printnb("\n");
	
//JJ1
	if ( (ret= dm_read_reg(dev, 0x29, &tmp)) >=0)
		printnb("++++++[dm962]+++++ dm_read_reg() 0x29 0x%02x\n",tmp);
	else
		printnb("++++++[dm962]+++++ dm_read_reg() 0x29 fail-func-return %d\n", ret);
		
	if ( (ret= dm_read_reg(dev, 0x28, &tmp)) >=0)
		printnb("++++++[dm962]+++++ dm_read_reg() 0x28 0x%02x\n",tmp);
	else
		printnb("++++++[dm962]+++++ dm_read_reg() 0x28 fail-func-return %d\n", ret);
		
	if ( (ret= dm_read_reg(dev, 0x2b, &tmp)) >=0)
		printnb("++++++[dm962]+++++ dm_read_reg() 0x2b 0x%02x\n",tmp);
	else
		printnb("++++++[dm962]+++++ dm_read_reg() 0x2b fail-func-return %d\n", ret);
	if ( (ret= dm_read_reg(dev, 0x2a, &tmp)) >=0)
		printnb("++++++[dm962]+++++ dm_read_reg() 0x2a 0x%02x\n",tmp);
	else
		printnb("++++++[dm962]+++++ dm_read_reg() 0x2a fail-func-return %d\n", ret);
	...
	printnb(KERN_WARNING "[dm96] DM9_NREV= %d\n", priv->tx_fix_mod);

  /*printnb("[dm96] Fixme: work around for 9620 mode\n");*/
  /*printnb("[dm96] Add tx_fixup() debug...\n");*/
	...
	if (temp == 0x02)
	{
	 ...
	 dm_read_reg(dev, DM_SMIREG, &temp);
	  printnb( "[dm96] 9620 REG91H (DM_SMIREG= Get 0x%02x, Xet xo 0xHH)\n", temp);
	 //dm_write_reg(dev, DM_SMIREG, temp &0xef);
      //printnb(KERN_WARNING "[dm96] 9620 REG91H (DM_SMIREG= Get 0x%02x, Set to 0x%02x)\n", temp, temp &0xef);
	 dm_read_reg(dev, DM_SMIREG, &temp);
      printnb( "[dm96] 9620 REG91H (DM_SMIREG= Get 0x%02x)\n", temp);

    /* bind version info.e */
     printnb("\n");
	 printnb("([dm962] Linux Driver = %s)\n", LNX_DM9620_VER_STR);
	printnb("[dm962] %s", LNX_CONF0_STR);
	 printnb("([dm962] Linux Driver OEM %s)\n", CONF_OEM);
     printnb("([dm962] Linux Driver INC %d EP3-INTVL 0x%02X)\n", priv->IP_conf_inc, WD12_EP3I); // 'CONF_INC'
	 printnb("\n");
	...
	dm9620_mdio_write(dev->net, dev->mii.phy_id, MII_ADVERTISE,
	 ...
	 dm_read_reg(dev, USB_CTRL, &temp);
      printnb(KERN_WARNING "[dm96] 9620 REGF4H (USB_CTRL= Get 0x%02x) (need 0x20 to have USB INT)\n", temp);
      
	mii_nway_restart(&dev->mii); 
	
out:
	//.printnb("dm9620_bind().e: return ret= %d\n", ret);
	return ret;
}
	...
	if (link == (int) netif_carrier_ok(dev->net))
	{
	  if (priv->desc_ep3i_pntonce==0)
	  {
	  	if (priv->desc_ep3i==0x07)
	  	  ...
	  	  dm9620_print(dev, "rpt: #dm9620_status_on ... netif_carrier %d ...\n", netif_carrier_ok(dev->net));
	  }
